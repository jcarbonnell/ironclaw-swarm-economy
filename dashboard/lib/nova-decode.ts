// lib/nova-decode.ts
//
// Browser-native decrypt + integrity hash for the NOVA Contributions panel.
// This is a VERBATIM PORT of the NOVA dashboard's proven nova-decode.ts — a
// behavioral match of nova-sdk-js's decode path (format.ts + legacy/v0.ts),
// deliberately NOT a dependency on the SDK. Do not re-derive or "improve" the
// crypto here: it is harness-verified byte-identical against real ciphertext.
// Importing nova-sdk-js would drag @near-js/providers + axios + the NovaSdk class
// (and a Buffer polyfill) into the browser bundle; this module needs none of
// that — it is pure browser primitives (crypto.subtle, DecompressionStream).
//
// Server never sees plaintext or the derived key beyond brokering the wrapped
// key + ciphertext (the prepare-retrieve route). Everything here runs in the
// authorized researcher's browser.
//
...
async function decryptV0(encryptedB64: string, keyB64: string): Promise<Uint8Array> {
  ...
  try {
    ...
    throw new NovaDecodeError(
      `key import failed: ${e instanceof Error ? e.message : String(e)}`
    );
  }
...
// SDK's compress path. Browser-native; no fallback needed.
